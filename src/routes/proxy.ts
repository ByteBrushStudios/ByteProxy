...
export const proxyRoutes = new Elysia({ prefix: '/proxy' })
    ...
    .get(
        ...
        },
        {
            detail: {
                ...
                description: 'Returns all configured services that can be used with the proxy',
                security: [{ bearerAuth: [] }],
                tags: ['Proxy'],
                responses: {
                    200: {
    ...
    .get(
        ...
        },
        {
            ...
            detail: {
                ...
                description: 'Returns the current rate limit status for a specific service',
                security: [{ bearerAuth: [] }],
                tags: ['Proxy'],
                responses: {
                    200: {
                    ...
                    404: {
                        description: 'Service not found or rate limiting not configured',
                        content: {
                            'application/json': {
                                schema: {
                                    type: 'object',
    ...
    .all(
        ...
        async ({ params, request, query }) => {
            const { service, '*': wildcardPath } = params
            ...
            if (service === 'github' && /^\/org\//.test(path)) {
                return new Response(
                    JSON.stringify({
            ...
            try {
                ...
                return new Response(typeof response.body === 'object' ? JSON.stringify(response.body) : response.body, {
        ...
        },
        {
            ...
            detail: {
                ...
                responses: {
                    200: {
                        description: 'Successful response from the proxied service',
                        content: {
                            'application/json': {
                                schema: {
                                    type: 'object',
                                    description: 'Response varies based on the proxied service'
                                }
                            }
                        }
                    },
                    ...
                    500: {
                        description: 'Internal server or proxy error',
                        content: {
                            'application/json': {
                                schema: {
                                    type: 'object',
    ...
    .get(
        ...
        },
        {
            detail: {
                ...
                description: 'Simple endpoint to verify that authentication is working correctly',
                security: [{ bearerAuth: [] }],
                tags: ['Proxy', 'Debug'],
                responses: {
                    200: {
        ...
        () => {
            ...
            return {
                ...
                authStatus: {
                    ...
                    setupStatus: {
                        proxy:
                            config.security.requireAuthForProxy && !proxyKeyConfigured
                                ? 'ERROR: Auth required but no key set'
                                : 'Valid configuration',
                        management:
                            config.security.requireAuthForManagement && !managementKeyConfigured
                                ? 'ERROR: Auth required but no key set'
                                : 'Valid configuration'
                    }
                },
                ...
                troubleshooting: {
                    noKeyProvided:
                        'If you received "No authentication provided", you need to add your API key using one of the methods above',
                    invalidKey:
                        'If you received "Valid API key required", your provided key doesn\'t match the one in your environment',
